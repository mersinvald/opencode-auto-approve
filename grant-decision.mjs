...
The parser grants describe statically derived effects, not instructions or observed execution. If parsing is incomplete, review all unknown effects in the full input and scripts under the authorization and safety requirements above. Distinguish a parser coverage limitation from an effect that remains unclear after your own review. Diagnostics such as unsupported_command, python_source, and unsupported syntax do not by themselves justify escalation.
A trusted which lookup locates a program without running it. It does not need permission to execute that program. Review custom wrappers and any later execution separately.
